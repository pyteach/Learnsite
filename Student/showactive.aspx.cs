...
public partial class Student_showactive : System.Web.UI.Page
{
                ...
                if (model.Mgroup)
                {
                    ...
                    if (sbll.IsLeader(sSnum))//如果是组长则显示小组面板
                    {
                        ...
                        if (gurl != "")
                        {
                            ...
                            {
                                Labelgroupmsg.Text = "小组作品未评价！<br/>你可以修改重新提交！";
                            }
                        }
                        else
                        {
        ...
        if (Wid != "")//判断有无作品提交
        {
            ...
            else
            {
                if (LearnSite.Common.XmlHelp.GetWorkIpLimit())//判断有无进行IP限制
                {
                    if (Snum == SnumDone || isTeacher(Wid, Snum))
                    {
                        Labelmsg.Text = "你已经提交作品.<br/>你可以修改重新提交！";
                    ...
                    else
                    {
                        ...
                        if (LabelMfiletype.Text != "htm")
                            Labelmsg.Text = SnumDone + "学号<br/>已经在该IP<br/>提交本活动作品.！";
                    }
                }
                else
                {
    ...
    private void showgroupWork()
    {
        ...
        LearnSite.BLL.Works wbll = new LearnSite.BLL.Works();
        GVgwork.DataSource = wbll.GetGroupWorks(aaSid, Int32.Parse(aaWmid));
        GVgwork.DataBind();
    }
